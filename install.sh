...
install_vim_plugins() {
    log_section "Neovim lazy.nvim"

    local LAZY_PATH="$HOME/.local/share/nvim/lazy/lazy.nvim"
    if [[ ! -d "$LAZY_PATH" ]]; then
        log_info "Installing lazy.nvim..."
        git clone --filter=blob:none https://github.com/folke/lazy.nvim.git --branch=stable "$LAZY_PATH" &>/dev/null
        log_ok "lazy.nvim installed"
    else
        log_skip "lazy.nvim already installed"
    fi

    # Ensure Lua config directory exists
    local LUA_DIR="$HOME/.config/nvim/lua"
    if [[ ! -d "$LUA_DIR" ]]; then
        mkdir -p "$LUA_DIR"
        log_ok "Created Neovim lua config directory"
    fi

    # Install plugins silently if nvim is available
